...
const OnlineExam = ({navigation}) => {
    ...
    const handleSubmit = ({subject, link}) => {
        ...
        actions.post(`/api/add-onlineexam`, {subject, link, postedBy: user.id}).then(res => {
            _panel.hide()
            onRefresh()
        }).catch(err => {
            console.log(err)
    ...
    const getClasses = () => {
        actions.get(`/api/get-exams/${user.id}`).then(res => {
            console.log(res.data)
            setExams(res.data.classes)
        }).catch(err => {
            console.log(err)
        })
    }   
    ...
    const handleDialog = () => {
        ...
        <Dialog.Description>
            Click on below url to open the Exam
        </Dialog.Description>
        <TouchableOpacity
